...
export async function loadBoard(force = false, options = {}) {
  const IS_CLOUD = Boolean(process.env.RENDER);
  const tier = options.tier === "fast" ? "fast" : "full";
  const fast = IS_CLOUD && tier === "fast";
  const enrichH2h = fast ? 20 : 40;
  const enrichFs = fast ? 20 : 40;
  const enrichExtra = fast ? 28 : 50;
  const cloudHint =
