...
export async function loadBoard(force = false) {
  ...
  if (!fourscore.ok && !(stavka.ok && stavka.matches?.length)) {
    return {
      ...
      error: fourscore.error || stavka.error || "Нет матчей",
      historyMatches: 0,
    };
  }
  ...
  try {
    matches = await enrichMatchesWithH2h(matches, { limit: 40 });
  } catch {
  ...
  try {
    matches = await enrichMatchesWithFourscoreStats(matches, { limit: 40 });
  } catch {
    // ignore
  }

  try {
    matches = await enrichMatchesWithExtraStats(matches, { limit: 50 });
  } catch {
    // ignore
  }
