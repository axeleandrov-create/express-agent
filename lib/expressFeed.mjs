...
export function buildCapperExpresses(_matches, learnBoost = {}, singlesRows = []) {
  ...
  for (const [key, profile] of Object.entries(CAPPER_PROFILES)) {
    const pool = buildCapperPool(profile, singlesRows);
    let coupon = buildCapperCoupon(pool, profile, singlesRows, usedPairs);
    if (!coupon) {
      coupon = buildCapperCoupon(pool, profile, singlesRows, new Set());
    }
    if (!coupon) {
      coupon = rebuildCapperLevel(key, singlesRows, [...usedPairs]);
    }
    out[key] = coupon;
    if (coupon?.matches) {
