...
function safeProfileVariants(base) {
  return [
    base,
...
function replaceProfileSteps(level, profile) {
  const loose = [
    {
      ...profile,
      minProb: Math.max(0.32, profile.minProb - 0.1),
      oddsMax: profile.oddsMax + 0.2,
    },
    {
      ...profile,
      outcomesOnly: false,
      minProb: Math.max(0.3, profile.minProb - 0.14),
      minTierA: Math.max(0, (profile.minTierA || 0) - 1),
      targetMin: Math.max(2, (profile.targetMin || 3) * 0.65),
      oddsMin: Math.max(1.25, profile.oddsMin - 0.08),
      oddsMax: profile.oddsMax + 0.5,
    },
  ];
  if (level === "safe") return [...safeProfileVariants(profile), ...loose];
  return [profile, ...loose];
}

function countNewLegs(coupon, previousKeys) {
  const prev = new Set((previousKeys || []).map(normKey));
  if (!prev.size || !coupon?.matches?.length) return coupon?.matches?.length || 0;
  return coupon.matches.filter((m) => !prev.has(teamKey(m))).length;
}
...
function buildCapperForLevel(key, singlesRows, usedPairs) {
  ...
  const profiles = key === "safe" ? safeProfileVariants(base) : [base];
...
export function rebuildCapperLevel(level, singlesRows = [], excludeKeys = [], options = {}) {
  ...
  const previousKeys = [...(options.previousKeys || options.mustChangeKeys || [])].map(normKey);
  const otherExclude = new Set((excludeKeys || []).map(normKey));
  const isReplace = previousKeys.length > 0;
  const minChangedLegs = Number(options.minChangedLegs) || (isReplace ? 1 : 0);
  const requireAllNew = Boolean(options.requireAllNew);
  const seed = Number(options.replaceSeed) || 0;

  const profileSteps = isReplace
    ? replaceProfileSteps(level, profile)
    : [
        profile,
        {
          ...profile,
  ...
  const tryBuild = (pool, prof, baseExclude) => {
    const maxRot = Math.max(32, pool.length * 6);
    ...
    for (let i = 0; i < maxRot; i++) {
      ...
      const coupon = buildCapperCoupon(pool, prof, singlesRows, baseExclude, rot);
      if (!coupon) continue;

      const changed = countNewLegs(coupon, previousKeys);
      if (isReplace) {
        if (requireAllNew && changed < coupon.matches.length) continue;
        if (changed < minChangedLegs) continue;
      }

      if (changed > bestChanged) {
        best = coupon;
        bestChanged = changed;
        if (!isReplace || changed === coupon.matches.length) break;
      }
    }
  ...
  for (const prof of profileSteps) {
    ...
    coupon = tryBuild(pool, prof, otherExclude);
    if (!coupon && isReplace) coupon = tryBuild(pool, prof, new Set());
    if (coupon) {
      changedLegs = countNewLegs(coupon, previousKeys);
      if (otherExclude.size) partial = true;
      if (isReplace && changedLegs < coupon.matches.length) partial = true;
      break;
    }
  }

  if (!coupon) return null;
  coupon._changedLegs = changedLegs;
